...
class DeviceUnBind(object):
    def __init__(self):
        try:
            self.data = {"status": None}
            self.BROKER_IP = "192.168.100.152"
            self.client = mqtt.Client("P5")
    ...
    def on_connect_finder(self, client, userdata, flags, rc):
        self.client.subscribe("zigbee2mqtt/bridge/response/device/unbind")
        print("connected! response2")
    ...
    def parse_data(self):
        if self.data["devices"]["status"] == "ok":
            return (
                f"Un-Binding successful for: {self.data['devices']['data']['clusters']}"
            )
